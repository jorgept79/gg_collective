...
@author: bowman
"""
#PIRMERO: SE CARGAN LOS MODELOS PREVIAMENTE GUARDADDOS (GENENADOR Y DISCRIMINADOR). HAY QUE ESPECIFICAR LAS RUTAS 
...
#TERCERO: SE PROMEDIAN LAS PÉRDIDAS DEL GENERADOR Y DISCRIMINADOR.
...
# Path a generador y discriminador elegidos
path_gen = "path/generator_landscape512.pt"  
path_critic="path/critic_landscapel512.pt"

# Path donde queramos guardar las imágenes
output_dir = "Images"


DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
LR = 1e-3
BATCH_SIZES = 32   #CAMBIAR EL MÚMERO DE IMÁGENES GENERADAS
CHANNELS_IMG = 3
Z_DIm = 512
W_DIM = 512
IN_CHANNELS = 512
...
#PRIMERO: SE CARGAN LOS MODELOS PREVIAMENTE GUARDADDOS (GENENADOR Y DISCRIMINADOR)
...
# Generar imágenes
with torch.no_grad():
    # Asumiendo un alpha y steps específicos.
    alpha = 1.0
    steps = 7
...
# Transformar los valores del tensor para que estén en el rango [0, 1] si es necesario
images = (images + 1) / 2

# Asegurarse de que el tensor de imagen tiene la forma (C, H, W)
# PyTorch utiliza esta convención pero algunas bibliotecas de visualización esperan (H, W, C)
...
checkpoint_gen = torch.load(path_gen, map_location=torch.device('cpu'))

opt_gen.load_state_dict(checkpoint_gen['optimizer_state_dict'])
epoch_gen = checkpoint_gen['epoch']
loss_gen = checkpoint_gen['loss']

# Load critic checkpoint
checkpoint_critic = torch.load(path_critic, map_location=torch.device('cpu'))


opt_critic.load_state_dict(checkpoint_critic['optimizer_state_dict'])
epoch_critic = checkpoint_critic['epoch']
loss_critic = checkpoint_critic['loss']
...
#TERCERO: SE PROMEMIAN LAS PÉRDIDAS DEL GENERADOR Y DISCRIMINADOR.
# Calcular el promedio de loss_gen
mean_loss_gen = statistics.mean(loss_gen)
print("El promedio de pérdida del generador es:", mean_loss_gen)

# Calcular el promedio de loss_critic
mean_loss_critic = statistics.mean(loss_critic)
print("El promedio de pérdida del discriminador es:", mean_loss_critic)